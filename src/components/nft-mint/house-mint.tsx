    ...
    <div className="flex flex-col animate-[fadeIn] duration-1200 rounded-[15px] overflow-hidden min-w-[257px]">
        ...
        <div className="flex items-end absolute w-full justify-between px-[10px] bottom-[10px]">
          <div>
            <span className="text-[#fff] text-[14px] font-semibold align-text-bottom leading-[18px]">
              x
              {numeral(Number(harvestAmount) >= 500 ? 6 : 5)
                .format("0.[0]")
                .toString()}{" "}
            </span>
      ...
      <div className="bg-[#6f8e9d66] p-[12px] flex flex-col p-3">
          ...
          <div className="text-[#0b6c96] text-[18px] font-semibold flex items-center">
            ...
            <Tooltip content="The number of NFT Credits to spend on the minting. NFT Credits can be earned by purchasing RWA Tokens.">
              ...
              <div>
                <BsInfoCircle
                  id="tooltip-icon"
                  className="w-4 h-4 cursor-pointer z-50 text-text-secondary"
                ></BsInfoCircle>
              </div>
            </Tooltip>
          </div>
        ...
        <Button
          ...
          className={`border-[1px] border-[#61cd81] w-full text-[16px] flex items-center bg-[#61cd81] justify-center mt-2 py-[5px] px-[20px] rounded-[20px] duration-400 h-[44px] disabled:bg-[#c2c5c3] ${BOLD_INTER_TIGHT.className}`}
        ...
        >
          {isLoading ? (
            <ReactLoading
              ...
              height="24px"
            />
          ) : Number(nftCreditCost) <= 0 ? (
            "Enter Credit Amount"
          ) : Number(nftCreditCost) < 400 ? (
            "Minimum 400 NFT Credits"
          ) : Number(nftCreditCost) > Number(nftCredits) ? (
            "Insufficient NFT Credits"
          ) : (
            "Mint"
          )}
        </Button>
