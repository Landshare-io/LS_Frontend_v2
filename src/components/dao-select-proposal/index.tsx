...
export default function DaoSelectProposal({ value, setValue }: DaoSelectProposalProps) {
  ...
  return (
    <>
      <div 
        className="w-full bg-[#ffffff] border-[1px] border-[#ffffff] rounded-[20px] py-[5px] pr-[5px] pl-[10px] flex justify-between items-center cursor-pointer relative md:w-[300px] hover:border-[#cdcdcd] hover:border-[#f2f4f5b3] duration-300 hover:bg-[#f2f4f5b3]" 
        onClick={() => setIsOpen(!isOpen)}
      >
        <div className="font-semibold text-[18px] leading-[20px] md:text-[18px] text-[#323232] py-[5px] pl-[10px] pr-[5px]">{value}</div>
        <Image src={ArrowDownIcon} alt="arrow-down" className='w-[30px] pr-[10px]' />
        <div className={`absolute bg-[#ffffff] shadow-lg shadow-[#0003] rounded-[12px] absolute z-[2] top-[38px] left-0 w-full ${!isOpen && 'hidden'}`}>
          <div 
