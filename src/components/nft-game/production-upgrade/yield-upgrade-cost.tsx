  ...
  return (
		...
		>
			<div className='flex flex-col text-[14px] px-[10px]'>
				{colorType < 2 || btnLabel != 'SALVAGE' ? (
					<div className='flex items-center'>
						...
						<div className='status-value'>
							...
							<span className='font-semibold text-[9px] leading-[18px] ml-[2px] text-[#323131] dark:text-text-secondary'>
								{' '}
								/year
							</span>
						</div>
					...
					<div className='flex items-center'>
						<span className='text-[#6f8e9d] font-semibold text-[10px] mb-0 ml-[4px] pr-[2px] dark:text-text-secondary'>
							Receive:{' '}
						</span>
									...
									.map((cost: number, index: number) => {
										if (Number(cost) > 0)
											return (
												<div
													key={`next-cost-${index}`}
													className='min-w-[20px] font-semibold mt-1 text-[14px] dark:text-text-primary'
												>
													{colorType == 0 ? (
				...
				<div className='border-b-[1px] border-[#00000050] dark:border-slate-300 w-full my-[8px]'></div>
			</div>
