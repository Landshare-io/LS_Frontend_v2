            ...
            <div className='flex flex-col px-[10px]'>
              <div className='flex justify-start items-center pt-[12px]'>
                ...
                {btnTitle == 'BUY' ? (
                  ...
                  <input
                    className='max-w-[50px] border-[1px] border-[#8d8d8d] dark:bg-[#49545b] dark:text-white rounded-[5px] text-right text-[0.8rem] px-[5px]'
                    type='number'
                    value={lumberCount}
              ...
              <div>
                {btnTitle === 'BUY' ? (
                        ...
                        .map((cost: number, index: number) => {
                          if (Number(cost) > 0)
                            return (
                              <div
                                key={`next-cost-${index}`}
                                className='min-w-[20px] flex gap-[2px] items-center text-[14px] font-medium'
                              >
                                {colorType == 0 ? (
                                  <>
                                    {`${cost} `}
                  ...
                  <div className='flex items-center'>
                    <span className='text-[#6f8e9d] text-semibold text-[10px] ml-[4px] pr-[2px] text-text-secondary'>
                      Receive:{' '}
                    </span>
                    <span className='flex gap-[2px] items-center font-semibold text-[14px] text-[#323131] text-text-primary'>
                      {numeral(Number(lumberCount) * item.buyReward[2]).format(
                        '0.[00]'
                      )}{' '}
                      {activeIcons[0]}
                    </span>
      ...
      >
        <div className='relative bg-white dark:bg-[#2e3740] rounded-xl shadow-2xl border border-gray-100 dark:border-gray-700 p-6 max-w-xs w-full m-auto'>
          ...
          <p className='text-sm text-gray-700 dark:text-gray-200 text-center leading-relaxed mt-2'>
            Burn lumber for power with the Firepit upgrade. Each lumber
            generates{' '}
            <span className='font-semibold text-blue-600 dark:text-blue-400'>
              {Number(item.buyReward[2])}
            </span>{' '}
            power.
          </p>
        </div>
