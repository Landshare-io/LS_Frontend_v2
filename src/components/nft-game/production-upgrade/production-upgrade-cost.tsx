...
export default function ProductionUpgradeCost({
  ...
  return (
    <div
      className={`flex flex-col w-full h-[130px] rounded-[25px] bg-[#fff] relative bg-primary ${color}`}
    >
      <div className='flex flex-col px-2'>
        {type == 'toolshed' && (
          <div className='flex items-center justify-between pt-[12px]'>
            {item.buyReward
              .slice(2, 7)
              .map((percent: number, index: number) => {
                if (Number(percent) > 0) {
                  return (
                    <div
                      className='flex items-center gap-[2px]'
                      key={`reduction-percent-${index}`}
                    >
                      <span className='flex items-center gap-[2px] text-[#6f8e9d] font-semibold text-[10px] ml-[4px] pr-[2px] dark:text-text-secondary'>
                        {disabledIcons[index]} repair:{' '}
                      </span>
                      <span className='font-semibold text-[14px] text-[#323131] text-text-primary'>{`-${percent * 100
                        }%`}</span>
                    </div>
                  );
                }
                ...
                return (
                  ...
                  >
                    <span className='text-[#6f8e9d] font-semibold text-[10px] ml-[4px] pr-[2px] dark:text-text-secondary'>
                      {disabledIcons[index]} Increase Production:{' '}
                    </span>
            ...
            {item.buy.slice(2, 7).map((cost: number, index: number) => {
              if (Number(cost) > 0)
                return (
                  <div
                    key={`next-cost-${index}`}
                    className={`min-w-[20px] text-[14px] flex gap-[2px] items-center`}
                  >
                    {colorType == 0 ? (
      ...
      <Button
        onClick={() => onPurcharse()}
        className={`w-full bottom-[-1px] h-[45px] text-[18px] font-semibold absolute rounded-[20px] ${isLoading.type > -1 && isLoading.type != type ? 'grey' : color
