...
export default function FeatureBadge({ icon, text, type, containerClassName }: FeatureBadgeProps) {
  return (
    <div className={`flex items-center gap-[8px] rounded-[50px] w-fit py-[4px] pl-[4px] pr-[20px] md:py-[6px] md:pl-[6px] ${type == 'gray' ? 'bg-primary' : 'bg-secondary'} ${containerClassName}`}>
      <div className={`w-[32px] h-[32px] p-[4px] rounded-[30px] ${type == 'gray' ? 'bg-[#ffffff] dark:bg-secondary' : 'bg-primary'}`}>
