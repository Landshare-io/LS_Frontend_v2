      ...
      <div className="relative rounded-[24px] shadow shadow-lg z-[3]" onClick={() => router.push(`/tokenized-asset/${property?.id}`)}>
        <SkeletonTheme baseColor={`${theme == 'dark' ? "#31333b" : "#dbdde0"}`} highlightColor={`${theme == 'dark' ? "#52545e" : "#f6f7f9"}`}>
          <div className="relative rounded-[24px] shadow shadow-lg z-[3] w-full md:w-[390px] md:h-[292px]">
            ...
            <span className={`absolute left-0 top-[20px] shadow-md rounded-r-[15px] text-[12px] leading-[20px] tracking-[0.02em] text-[0.02em] text-[#fff] py-[4px] px-[15px] ${property.type == "Rental Property" ? "bg-[#FF5454]" : "bg-[#3B98EE]"} ${BOLD_INTER_TIGHT.className}`}>
              {property.type}
            </span>
          </div>
          <div className="flex flex-col py-[20px] px-[30px] gap-[24px] bg-third rounded-b-[24px]">
              ...
              <div className="flex gap-[10px] md:gap-[20px]">
                {property?.bedrooms &&
                  <div className="flex items-center gap-[5px]">
                    ...
                    <div className="text-[12px] leading-[20px] text-[14px] font-medium leading-[22px] text-text-primary">{property?.bedrooms}</div>
                  </div>
                }
                {property?.bathrooms &&
                  <div className="flex items-center gap-[5px]">
                    <Image src={IconBathroom} alt="bathroom" />
                    <div className="text-[12px] leading-[20px] text-[14px] font-medium leading-[22px] text-text-primary">{property?.bathrooms}</div>
                  </div>
                }
              </div>
            </div>
