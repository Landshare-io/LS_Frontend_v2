    ...
    <div className="property-card-wrapper cursor-pointer">
      <div className={`property-card-container ${active ? 'active' : ''}`} style={style} onClick={() => router.push(`/tokenized-asset/${property?.id}`)}>
        <SkeletonTheme baseColor={`${isDarkMode ? "#31333b" : "#dbdde0"}`} highlightColor={`${isDarkMode ? "#52545e" : "#f6f7f9"}`}>
          ...
          <div className={`property-card-description-section ${active ? 'bg-third' : ''}`}>
            <div className="property-card-price-wrapper">
              <div className="property-card-price-detail">
                <span className="property-card-label text-text-third">Property Value</span>
                {isLoading ? (<Skeleton className="rounded-lg" width={80} height={26} />) : (<span className="property-card-value text-text-primary">${propertyValue.toLocaleString()}</span>)}
              </div>
              <div className="property-card-price-detail">
                <span className="property-card-label text-text-third">Rental Yield</span>
                {isLoading ? (<Skeleton className="rounded-lg" width={80} height={26} />) : (<span className="property-card-value text-text-primary">{((property.grossRent * (1 - property.management) * 12 - property.insurance - property.tax) / propertyValue * 100).toFixed(2)}%</span>)}
              </div>
              <div className="property-card-price-detail">
                <span className="property-card-label text-text-third">Ann. Return</span>
                {isLoading ? (<Skeleton className="rounded-lg" width={80} height={26} />) : (<span className="property-card-value text-text-primary">{((property.grossRent * (1 - property.management) * 12 - property.insurance - property.tax) / propertyValue * 100 + property.appreciation).toFixed(2)}%</span>)}
              </div>
            </div>
            <div className="property-card-details-wrapper">
              <span className="property-card-type bg-primary">Single Family</span>
              <div className="property-card-inventories">
                {property?.bedrooms &&
                  <div className="property-card-furniture">
                    <img src={IconSofa} alt="sofa" />
                    <div className="text-text-primary">{property?.bedrooms}</div>
                  </div>
                }
                {property?.bathrooms &&
                  <div className="property-card-furniture">
                    <img src={IconBathroom} alt="sofa" />
                    <div className="text-text-primary">{property?.bathrooms}</div>
                  </div>
                }
              </div>
            </div>
