...
export default function StatusCard() {
  ...
  const { data: landAmountInLp } = useBalanceOf({
    chainId,
    address: LP_TOKEN_V2_CONTRACT_ADDRESS[bsc.id],
  }) as { data: BigNumberish };
  const { data: totalBNBinLp } = useWBNBBalanceOf({
    chainId,
    address: LP_TOKEN_V2_CONTRACT_ADDRESS[bsc.id],
  }) as { data: BigNumberish };
  const { data: totalLpInVault } = useLpTokenBalanceOf({
    chainId,
    address: MASTERCHEF_CONTRACT_ADDRESS[bsc.id],
  }) as { data: BigNumberish };
  const { data: totalLpSupply } = useTotalSupply(chainId) as {
    data: BigNumberish;
  };
...
const vaultBalance =
  ...
  return (
    <div className="bg-primary">
      <div className="flex w-full justify-center items-center px-[20px] md:translate-y-[60px]">
        <div className="max-w-[1200px] w-full bg-secondary py-[30px] grid grid-cols-4 justify-between px-[16px] lg:px-[0px] gap-y-[40px] relative border-2 border-[#fff] dark:border-[#42444d] rounded-[20px] shadow shadow-lg">
          ...
          <div className="col-span-2 md:col-span-1 h-full flex flex-col justify-center space-y-2 p-[0px] border-r-[0px] border-[#61cd81] md:border-r-[2px]">
            <div className="w-full text-center text-xl md:text-2xl font-bold flex justify-center text-text-primary">
              <Counter
                duration={10}
                number={abbreviateNumber(Number(apy.toFixed(2)))}
                decimal={2}
                formatType={"percent"}
              ></Counter>
            </div>
            <div className="flex items-end justify-center gap-[4px] w-full text-center text-[12px] md:text-sm text-text-secondary font-medium">
              Staking APY{" "}
              <Link href="/vaults">
                <FiExternalLink className="text-[#61cd81] mb-1" />
              </Link>
            </div>
          </div>
          ...
          <div className="col-span-2 md:col-span-1 h-full flex flex-col justify-center space-y-2 p-[0px] border-r-[0px] border-[#61cd81] md:border-r-[2px]">
            <div className="w-full text-center text-xl md:text-2xl font-bold flex justify-center text-text-primary">
              <Counter
                duration={10}
                number={abbreviateNumber(Number(apr.toFixed(0)))}
                decimal={2}
                formatType={"percent"}
              ></Counter>
            </div>
