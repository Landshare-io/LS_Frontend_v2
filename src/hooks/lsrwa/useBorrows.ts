...
export function useBorrows() {
  ...
  const fetchBorrowers = async(vault : any) => {
  ...
  const fetchBorrows = async (signer : any, page=1, limit=10, owner='', isAdmin=true) => {
    ...
    if (borrowers.length > 0) {
      const data = await vault.getBorrowRequests(borrowers);
        ...
        requests = data.map((item : any, index : number) => ({
          ...
          amount: formatUnits(item[0], parseInt((process.env.NEXT_PUBLIC_USDC_DECIMALS as any))),
          repaid: item[1],
          approved: item[2]
        }));
        requests = requests.filter((item : any) => !item.repaid)
