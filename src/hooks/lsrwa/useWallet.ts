...
export function useWallet() {
  ...
  return {
    ...
    connector,
    disconnect,
    balance: Number(usdcBalance?.value ? numeral(formatUnits(usdcBalance.value, 6)).format("0.[000]") : '0.0'),
    symbol: usdcBalance?.symbol ?? '',
    tokenBalance: Number(!isTokenBalanceLoading ? numeral(formatUnits((tokenBalance as any).value, 18)).format("0.[000]") : '0.0'),
